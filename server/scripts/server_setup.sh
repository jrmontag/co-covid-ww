...
# clone the repo:
cd /apps
git clone https://github.com/jrmontag/co-covid-ww app_repo
...
cp /apps/app_repo/server/units/wastewater.service /etc/systemd/system/
...
cp /apps/app_repo/server/nginx/wastewater.nginx /etc/nginx/sites-enabled/
...
15 0 * * * cd /apps/app_repo && . ../venv/bin/activate && python tools/update_data.py >>data/cron.log 2>&1
